...
class s3_utils(object):
    ...
    def __init__(self, key, secret):
        ...
        self.key = key
        self.secret = secret
        self.session = start_session(aws_access_key_id, aws_secret_access_key, region_name)
    ...
    def get_bucket_key(self, bucket_name, key_name):
        self.current_bucket_key=  self.client().get_object(Bucket=bucket_name, Key=key_name)
    ...
    def to_s3(self, bucket_name, data, key_name):
        response = self.client().put_object(Bucket=bucket_name,
    ...
    def from_s3(self, bucket_name, key_name):
        return pickle.loads(zlib.decompress(self.client().get_object(Bucket=bucket_name, Key=key_name)['Body'].read()))
    ...
    def from_bin_file_streaming(self, name, bucket_name, key_name, full_path=False):
        ...
        with open(path, 'ab') as f:
            obj = self.client().get_object(Bucket=bucket_name, Key=key_name)['Body'].iter_lines()
            for i in obj:
                f.write(i)
